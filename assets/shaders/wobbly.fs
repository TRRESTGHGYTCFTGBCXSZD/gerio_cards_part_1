...
vec2 clamp(vec2 base, vec2 min, vec2 max)
{
	vec2 result = base;
	if (result.x < min.x) {
		result.x = min.x;
...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
	...
	//vec2 inverseuv = (texture_details.ba*(texture_coords+texture_details.xy))/image_details;


    //vec2 rotater = vec2(cos(wobbly.r*0.1221), sin(wobbly.r*0.3512));

    // Take pixel color (rgba) from `texture` at `texture_coords`, equivalent of texture2D in GLSL
	vec2 tc = texture_coords;
	tc.x += sin(wobbly.g*7.+uv.y*8.*3.1415926535897932384626433832795)/(image_details.x/texture_details.b*36.);
	tc.y += cos(wobbly.g*11.+uv.x*8.*3.1415926535897932384626433832795)/(image_details.y/texture_details.a*36.);
	tc = clamp(tc, (texture_details.xy*texture_details.ba)/image_details, ((texture_details.xy*texture_details.ba)+texture_details.ba)/image_details);
